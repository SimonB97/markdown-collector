...
export function loadMarkdownData(
  ...
  openUrls,
  query = "",
  filters = { searchUrl: true, searchTitle: true, searchContents: true }
) {
...
function renderMarkdownData(markdownData, container, openUrls, query, filters) {
  ...
  if (markdownData && markdownData.length > 0) {
    const filteredData = query
      ? searchMarkdownEntries(markdownData, query, filters)
      : markdownData;
    ...
    dateGroups.forEach((date) => {
      ...
      dateContent.style.display =
        date === getTodayDate() || query ? "block" : "none"; // Ensure date headers are expanded when search results are displayed
      ...
      dateCheckbox.addEventListener("change", () => {
        ...
        checkboxes.forEach(
          (checkbox) => (checkbox.checked = dateCheckbox.checked)
        );
      });
        ...
        if (item.isLoading) {
          ...
          textarea.addEventListener("blur", () => {
            ...
            browser.storage.local
              ...
              .then((result) => {
                let { markdownData } = result;
                const itemIndex = markdownData.findIndex(
                  (md) => md.url === item.url
                );
                if (itemIndex !== -1) {
...
export function fetchAndConvertToMarkdown(url, enableCleanup, callback) {
  ...
  Promise.race([fetchPromise, timeoutPromise])
    ...
    .then((html) => {
      return new Promise((resolve) => {
        if (enableCleanup) {
          ...
          readabilityScript.onload = () => {
            document.head.removeChild(readabilityScript); // Remove script after loading
          };
        } else {
          resolve(html);
        }
    ...
    .then((html) => {
      ...
      script.onload = () => {
        document.head.removeChild(script); // Remove script after loading
      };
...
function createPlusIcon(batchInfo) {
  ...
  function showPopup() {
    if (!popup) {
      popup = createHoverPopupWithListeners(batchInfo);
  ...
  const createHoverPopupWithListeners = (batchInfo) => {
    const popup = createHoverPopup(batchInfo);
